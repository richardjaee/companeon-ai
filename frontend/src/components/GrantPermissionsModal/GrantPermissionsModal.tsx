...
export default function GrantPermissionsModal({
  ...
  const handleStartGrantPermissions = async () => {
    try {
      ...
      // Step 0: Switch to Sepolia FIRST before any Flask detection
      // ERC-7715 only works on Sepolia, and wallet_getCapabilities behaves differently on different chains
    ...
    try {
      ...
      setTimeout(() => {
        handleRegisterWalletAgent(
          ...
          smartAccountResult.allPermissionContexts || null // NEW: All contexts by token
        );
      }, 500);
  ...
  const handleRegisterWalletAgent = async (
    ...
    allContexts: Record<string, string> | null // NEW: All permission contexts by token
  ) => {
    try {
      ...
      const requestBody = {
        ...
        allPermissionContexts: allContexts, // NEW: All contexts keyed by token address ('native' for ETH)
        delegationManager: delegManager,
        chainId: 11155111, // Sepolia - ERC-7715 only works on Sepolia
        expiresAt: expiresAt,
        smartAccountAddress: smartAccount,
        scopes: scopes // Include the scopes so backend can track limits
      };
