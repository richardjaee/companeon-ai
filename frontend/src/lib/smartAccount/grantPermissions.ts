 ...
 * Uses the EXACT format from your working GrantPermissionsModal
 */
export async function grantERC7715Permissions(
  ...
  // Step 5: Build ERC-7715 permissions array (EXACT format from working code!)
 ...
 * @param scopes - Array of permission scopes (MUST match what was sent to MetaMask)
 */
export async function registerWalletAgent(
  ...
  const requestBody = {
    ...
    scopes: scopes // Include scopes so backend can track limits
  };
