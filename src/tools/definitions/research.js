 ...
 * 
 * NEW SIMPLIFIED FLOW (using ERC-7715 delegation):
 * 1. pay_x402 - Uses delegation to transfer USDC directly from user's wallet to x402 provider
 * 2. web_research - Calls Perplexity with the paid access
 * 
 * No need to fund signer wallet! Delegation handles everything.
 */
...
async function generateImage(prompt, logger, emit) {
  const projectId = process.env.GOOGLE_CLOUD_PROJECT || 'bvgs-prod';
